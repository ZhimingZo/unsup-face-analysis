...
from eval.eval_aflw import evaluate_aflw
...
def train(args, data_loader, network, optimizer, scheduler, ckpt=None):
    ...
    loss_l1 = nn.L1Loss()
    vgg_net = Vgg16().cuda()
    save_path = os.path.join("checkpoint/end_to_end_AFLW/", args.model_name, str(args.n_maps))
    ...
    for e in range(args.epoch): 
        ...
        for i, (source_img, target_img,  input_img, mask, _)  in enumerate(data_loader):
	...
	# save model 
        ...
        # evaluation 
        model_error_aflw = evaluate_aflw(network)
        #angle_error = eval_angle(network)
        print("model_error_on_aflw: " + str(model_error_aflw)) 
        if best_model_error_aflw is None or model_error_aflw < best_model_error_aflw:
            best_model_error_aflw = model_error_aflw
            # save best pth
            torch.save({
                'epoch': e,
                'model_state_dict': network.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'loss': total_loss_AM.avg,
            }, os.path.join(save_path, 'best_model_aflw'+ '.pth'))
        F.write('Epoch: ' +str(e+1) + '\t' + 'Loss: ' +str(total_loss_AM.avg) + '\t ' + "kps_loss: "+ str(kps_loss_AM.avg)  + "\t" + "aflw_error: " + str(model_error_aflw))
        F.write('\n')
        F.flush()
    F.close()
...
def main(): 
    ...
    parser.add_argument('--model_name', default='awareNet_end_to_end_aflw_kps10_v2',type=str, help='model_name')
    ...
    parser.add_argument('--epoch', default=50, type=float, help='training epochs')
    ...
    dataset = import_dataset_aflw(True)
