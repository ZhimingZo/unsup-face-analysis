...
class MAFLImageDataset(Dataset): 
    ...
    def __getitem__(self, idx):
           ...
           mask = _get_smooth_mask(self.img_size[0], self.img_size[1], 10, 20)[:, :, None] #128 * 128 * 1 
...
def import_dataset_mafl(is_train):
    annotations = np.load('./Data/celeba/mafl_keypoints.npz')
    if is_train: 
        annotation=annotations['train_kps']
        img_csv_file = './Data/celeba/mafl_training_dataset.csv'
    ...
    dataset = MAFLImageDataset(annotations=annotation, img_csv_file=img_csv_file, img_dir=img_dir, transform=transform)
    return dataset
